...
import { analyzeProject, analyzeProjectInteractive, getProject, type AnalyzeResponse } from './api'
...
export default function App() {
    ...
    const [analysisMode, setAnalysisMode] = useState<AnalyzeMode>('standard')

    const handleAnalyze = useCallback(async (path: string, mode: AnalyzeMode) => {
        ...
        const stages = [
            ...
            { pct: 75, text: mode === 'interactive' ? '交互式拉取关键代码...' : 'AI 生成审计报告...' },
            { pct: 85, text: mode === 'interactive' ? '整理交互式审计结果...' : '向量化索引（RAG）...' },
        ]
        ...
        try {
            const data: AnalyzeResponse = mode === 'interactive'
                ? await analyzeProjectInteractive(path)
                : await analyzeProject(path)
            window.clearTimeout(timerGuard)
            if (timer != null) window.clearInterval(timer)

            const pid = mode === 'standard' ? (data.projectId || '') : ''
            setProjectId(pid)
            if (mode === 'standard' && pid) localStorage.setItem('projectId', pid)

            setProgress(100)
            setProgressText(
                mode === 'interactive'
                    ? '交互式分析完成 ✓'
                    : (data.alreadyIndexed ? '已存在索引，加载完成 ✓' : '分析完成 ✓')
            )
            ...
            <main className="flex-1 flex flex-col">
                ...
                {phase === 'done' && (
                    <div className="flex-1 flex flex-col lg:flex-row gap-0 lg:gap-1 h-[calc(100vh-64px)]">
                        {/* 左：审计报告 */}
                        <div className={analysisMode === 'standard' && projectId ? 'lg:w-1/2 h-1/2 lg:h-full overflow-hidden flex flex-col' : 'w-full h-full overflow-hidden flex flex-col'}>
                            <BlueprintViewer
                                content={blueprint}
                                filesScanned={filesScanned}
                            />
                        </div>
                        {/* 右：RAG 对话（仅标准模式且有 projectId 时显示） */}
                        {analysisMode === 'standard' && projectId && (
